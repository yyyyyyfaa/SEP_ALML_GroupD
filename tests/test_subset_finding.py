...
def generate_interaction_index(n_players: int, max_order: int, min_order: int):
    ...
    return list(
        chain.from_iterable(
            combinations(range(n_players), k) for k in range(min_order, max_order + 1)
        )
    )
...
def test_v_hat_with_k_max_zero():
    ...
    weights = {frozenset(): 1.0, frozenset([0]): 2.0}
...
def test_subset_finding_with_all_zero_values():
    ...
    values = [0.0, 0.0, 0.0]
    iv = InteractionValues(
        index=index, values=values, n_players=2, max_order=2, min_order=1, baseline_value=0.0
    )
    result = subset_finding(iv, max_size=2)
...
def test_subset_finding_with_simple_imput():
    ...
    index = frozenset(index)
    iv = InteractionValues(
        index=index, values=[], n_players=0, max_order=0, min_order=0, baseline_value=0.0
    )
