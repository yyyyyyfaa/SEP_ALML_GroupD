...
class GaussianConditionalImputer(Imputer):
    ...
    def transform(self, X: np.ndarray, mask: np.ndarray = None) -> np.ndarray:
        ...
        for i in range(n_samples):
            ...
            mu = self.mean
            Σ = self.CovMatrix
            Σ_CC = Σ[np.ix_(observed_idx, observed_idx)]
            Σ_MC = Σ[np.ix_(missing_idx,   observed_idx)]
            Σ_MM = Σ[np.ix_(missing_idx,   missing_idx)]

            # 3. compute mean value
            x_C = X_imp[i, observed_idx]
            mu_C = mu[observed_idx]
            mu_M = mu[missing_idx]
            Σ_CC_inv = inv(Σ_CC)  # 确保 Σ_CC 可逆；有需要可加微小正则项
            cond_mean = mu_M + Σ_MC.dot(Σ_CC_inv.dot(x_C - mu_C))

            # 4. compute covariance
            cond_cov = Σ_MM - Σ_MC.dot(Σ_CC_inv.dot(Σ_MC.T))

            # 5. use mean value to fill missing value
            X_imp[i, missing_idx] = cond_mean
